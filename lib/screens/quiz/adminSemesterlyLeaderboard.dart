...
class AdminSemesterlyLeaderboard extends StatefulWidget {
  ...
  @override
  _AdminSemesterlyLeaderboardState createState() => _AdminSemesterlyLeaderboardState();
}

// Widget to display the Pub Quiz semesterly leaderboards as scrollable lists
...
// And a pop-up form to end the current season
class _AdminSemesterlyLeaderboardState extends State<AdminSemesterlyLeaderboard> {
  ...
  Future _retrieveCurrentSemesterlySeason() async {
    seasonNumber = await _database.retrieveCurrentSemesterlySeason();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        future: _retrieveCurrentSemesterlySeason(),
                ...
                body: Column(
                  children: [
                    ...
                    Padding(
                      ...
                      child: buildTabTitle('Semesterly Leaderboard', 25),
                    ),


                    StreamBuilder<QuerySnapshot>(
                      // Set the stream to listen to the singleton semesterly
                      // leaderboard document
                      // So that the current season's prize can be retrieved
                        stream: _database.getSemesterlyLeaderboardDoc(),
                          ...
                          return Padding(
                            ...
                            child: ListView(
                                ...
                                children: snapshot.data!.docs
                                  ...
                                  SemesterlyLeaderboardDoc semesterlyDoc = _database
                                      .semesterlyLeaderboardDocFromSnapshot(data);

                                  // Update the current season variable each time the
                                  // current season changes in the database
                                  seasonNumber = semesterlyDoc.currentSeason!;

                                  // Update currentPrize for use in the Edit Prize
                                  // pop-up form
                                  currentPrize = semesterlyDoc.prizes![
                    ...
                    StreamBuilder<QuerySnapshot>(
                      // Set the stream to listen to the semesterly leaderboard
                      // entries documents
                        stream: _database.getSemesterlyScores(
                            ...
                            child: Padding(
                              ...
                              child: ListView(
                                  ...
                                  children: snapshot.data!.docs
                                    ...
                                    // Do not display the singleton semesterly
                                    // leaderboard doc in the list
                                    // '2RpLeBNHCgOcHzjORDCQ' is the ID of this
                                    // document
                                    if (leaderboardEntry.id !=
                                        '2RpLeBNHCgOcHzjORDCQ') {
                                      ...
                                      // the SemesterlyLeaderboardEntry document
                                      // This is done to discover the team name related to the
                                      // SemesterlyLeaderboardEntry document
                                      return FutureBuilder(
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: ElevatedButton(
                            ...
                            // When the End Season button is tapped
                            // launch its pop-up form
                            onPressed: () async {
                              showEndSeasonDialog(context, seasonNumber, rebuildWidgetTree);
                            },
                          ),
                        ),
                      ],
  ...
  Widget submitButton = TextButton(
    ...
    onPressed: () async {
      ...
      if (_formKey.currentState!.validate()) {
        ...
        // Update the semesterly prize in the database
        await _database.updateSemesterlyPrize(prizeText, seasonNumber);
...
// The End Season pop-up form
showEndSeasonDialog(context, seasonNumber, rebuildWidgetTree) {
  final DatabaseService _database = DatabaseService();

  // If the user taps "Yes", end the current season and start the next one
  // Then close the pop-up
  // Then redirect the user to the leaderboard admin menu
  // Then rebuild the Widget Tree
  Widget yesButton = TextButton(
    child: const Text("Yes"),
    onPressed: () async {

      // End the current season and start the next one
      await _database.endSemesterSeason();

      // Close the pop-up
      Navigator.of(context).pop();

      // Rebuild the Widget Tree to allow the new season to be correctly
      // displayed
      rebuildWidgetTree();
    },
  );
  ...
  AlertDialog alert = AlertDialog(
    ...
    content: const SizedBox(
      height: 100,
      child: Text('Are you sure you want to end the current season?')
    ),
    actions: [
      yesButton,
      cancelButton,
  ...
  // The function to build the End Season dialog box
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return alert;
